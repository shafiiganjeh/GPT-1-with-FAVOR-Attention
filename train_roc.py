...
if __name__ == '__main__':
    ...
    parser.add_argument('--prep', action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument('--lora', action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument('--FAVOR', action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument('--random_features', type=int, default=32)
    parser.add_argument('--save_dir', type=str, default='Files')
    parser.add_argument('--encoder_dir', type=str, default='Files')
    parser.add_argument('--data_dir', type=str, default='/home/borz/Desktop/svd')
    ...
    parser.add_argument('--n_ctx', type=int, default=56)
    parser.add_argument('--lora_dim', type=int, default=4  )
    parser.add_argument('--n_embd', type=int, default=768)
    parser.add_argument('--n_head', type=int, default=12)
    parser.add_argument('--n_layer', type=int, default=12)
    parser.add_argument('--batch', type=int, default=16)
    parser.add_argument('--freeze_emb', action=argparse.BooleanOptionalAction, default=False)
    ...
    text_encoder = TextEncoder(encoder_dir + '/encoder_bpe_40000.json',encoder_dir + '/vocab_40000.bpe')
    ...
    if prep:
        ...
        dt = rocstories(path_val = save_dir+str("/") + val_name, path_test = save_dir+str("/") + test_name, n_train = n_train, n_valid = n_valid)
    ...
    if FAVOR:
        if lora:
            learning_rate = ExpSchedule(warmup_steps = 20,decay = .004,lr = 1e-3)
            weight_decay = 0.01
        else:
        ...
        if lora:
            learning_rate = ExpSchedule(warmup_steps = 20,decay = .001,lr = 6.25e-4)
            weight_decay = 0.01
        else:
            learning_rate = LinearSchedule(warmup_steps = 4,decay = 93*4,lr = 6.25e-5)
            weight_decay = None
    ...
    @tf.function
    def train_step(x_batch_train, y_batch_train,model,labda = .5):
      ...
      with tf.GradientTape() as tape:
        ...
        train_loss = tf.reduce_mean(l2) + labda* tf.reduce_mean(l1)
    ...
    for epoch in range(epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train):
            ...
            l2 = train_step(x_batch_train, y_batch_train,model = model,labda = l)

            l_cum[0] = tf.reduce_mean(l2)
            l_cum = np.roll(l_cum, 1)

            sys.stdout.write('\r'+("epoch: " + str(epoch+1) + " step: " +str(step+1) + "/" + str(train.cardinality().numpy()) + " loss: " + str(np.sum(l_cum)/25) + " memory: " + str(round(tf.config.experimental.get_memory_info('GPU:0')["peak"]*1e-6)) + "   "))
